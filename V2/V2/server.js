...
wss.on('connection', (ws) => {
  scaleClients.add(ws);
  console.log(`📡 WebSocket client connected. Total clients: ${scaleClients.size}`);
  
  // Send initial connection confirmation with config info
  try {
    ws.send(JSON.stringify({ 
      ...
      config: {
        ...
        passiveMode: scaleConfig.passiveMode || false,
        timeoutMs: scaleConfig.timeoutMs || 2000
      }
    }));
  } catch (err) {
    console.error('❌ Error sending WebSocket welcome message:', err.message);
  }
  
  // Start continuous reading if not already active
  if (!continuousReadingActive && scaleClients.size > 0) {
    console.log('🔄 Starting continuous reading for WebSocket clients...');
    startContinuousReading();
  } else if (continuousReadingActive) {
    console.log('✅ Continuous reading already active, WebSocket will receive data');
  }
  
  ws.on('close', () => {
    scaleClients.delete(ws);
    console.log(`📡 WebSocket client disconnected. Remaining clients: ${scaleClients.size}`);
    
    // Stop continuous reading if no clients remain
    if (scaleClients.size === 0 && continuousReadingActive) {
      console.log('🛑 No WebSocket clients remaining, stopping continuous reading');
...
function broadcastScaleData(data) {
  if (scaleClients.size === 0) {
    if (DEBUG_SCALE) console.log('⚠️ No WebSocket clients to broadcast to');
    return;
  }
  ...
  scaleClients.forEach((client) => {
    if (client.readyState === WebSocket.OPEN) {
      try {
        ...
        console.error('❌ Error sending WebSocket message:', error.message || error);
  ...
  if (DEBUG_SCALE && sentCount > 0) {
    console.log(`📤 Broadcasted scale data to ${sentCount} client(s)${errorCount > 0 ? `, ${errorCount} error(s)` : ''}`);
  }
}
...
let lastScaleReading = null; // Cache last reading for /api/scale/read when continuous reading is active

function startContinuousReading() {
  if (continuousReadingActive) {
    console.log('⚠️ Continuous reading already active, skipping start');
    return;
  }
  if (!SerialPortLib || !scaleConfig.port) {
    console.error('❌ Cannot start continuous reading: SerialPortLib or port not available');
    return;
  }
  
  continuousReadingActive = true;
  const isPassiveMode = scaleConfig.passiveMode === true;
  console.log(`🔄 Starting continuous reading mode for WebSocket clients (passiveMode=${isPassiveMode})`);
  
  // Ensure port is open
  ensurePortOpen().then(() => {
    if (!activePort || !activePort.isOpen) {
      console.error('❌ Failed to open port for continuous reading');
      continuousReadingActive = false;
      return;
    }
    
    console.log(`✅ Port opened for continuous reading: ${scaleConfig.port}, isOpen=${activePort.isOpen}`);
    
    // Set up data handler for continuous reading
    const dataHandler = (chunk) => {
      // Reduced logging: only log occasionally to reduce console spam (every 50th chunk)
      if (Math.random() < 0.02) {
        console.log(`📥 Raw data received from scale (${chunk.length} bytes):`, chunk.toString('hex'), '| ASCII:', JSON.stringify(chunk.toString('ascii')));
      }
      ...
      while (newlineIdx >= 0) {
        ...
        if (line.length > 0) {
          ...
          // Reduced logging: only log occasionally (every 20th line)
          if (Math.random() < 0.05) {
            console.log(`📝 Processing line from scale:`, JSON.stringify(trimmed));
          }
          
          if (trimmed.length > 0) {
            const parsed = parseWeightSmart(trimmed);
            if (parsed) {
              // Cache last reading for /api/scale/read endpoint
              lastScaleReading = parsed;
              // Only log occasionally to reduce console spam (every 20th reading)
              if (Math.random() < 0.05) {
                console.log(`✅ Parsed weight:`, parsed);
              }
              broadcastScaleData(parsed);
            } else {
              if (Math.random() < 0.1) { // Only log 10% of parse failures
                console.warn(`⚠️ Failed to parse weight from line:`, JSON.stringify(trimmed));
              }
            }
          }
        }
    ...
    // ALTERNATIVE: Passive mode vs Active polling mode
    const isPassiveMode = scaleConfig.passiveMode === true;
    if (!isPassiveMode) {
      // Active mode: Poll for data every 50ms (20 times per second) - seamless real-time updates
      console.log('📤 Active mode: Starting polling interval (50ms)');
      continuousReadingInterval = setInterval(() => {
        if (activePort && activePort.isOpen && scaleClients.size > 0) {
          try {
            const pollCmd = scaleConfig.pollCommand || 'P\r';
            // Reduced logging: only log occasionally (every 200th poll = ~10 seconds)
            if (Math.random() < 0.005) {
              console.log(`📤 Sending poll command:`, JSON.stringify(pollCmd), `(hex: ${Buffer.from(pollCmd).toString('hex')})`);
            }
            activePort.write(pollCmd);
          } catch (writeErr) {
            console.warn('⚠️ Poll write error in continuous reading:', writeErr.message);
          }
        } else {
          // Port closed or no clients - stop polling
      ...
      // Passive mode: Don't poll, just listen for data that scale sends automatically
      // Some scales (like AND EK-15KL) send data continuously without needing poll commands
      console.log('📡 Passive mode: Continuous reading active - waiting for scale to send data automatically');
      console.log('   Data handler is listening on port, will broadcast to WebSocket clients when data arrives');
      // No interval needed - data handler will process incoming data automatically
    }
  }).catch((error) => {
    console.error('❌ Failed to start continuous reading:', error.message || error);
    ...
    const errorMessage = JSON.stringify({
      type: 'error',
      message: 'Failed to start continuous reading',
    ...
    scaleClients.forEach((client) => {
      if (client.readyState === WebSocket.OPEN) {
        try {
          ...
          console.error('❌ Error sending error message to client:', sendErr.message);
        }
      }
    });
  });
...
function stopContinuousReading() {
  if (!continuousReadingActive) {
    console.log('⚠️ Continuous reading not active, nothing to stop');
    return;
  }
  
  console.log('🛑 Stopping continuous reading mode');
  continuousReadingActive = false;
  ...
  // CRITICAL: In passive mode, we should NOT remove data listeners
  // because port might still be needed for HTTP polling
  // Only remove if we're sure no other process needs the port
  if (activePort && !scaleConfig.passiveMode) {
    // In active mode, it's safe to remove listeners when stopping
    activePort.removeAllListeners('data');
    console.log('   ✅ Data listeners removed (active mode)');
  } else if (activePort && scaleConfig.passiveMode) {
    console.log('   ⚠️ Keeping data listeners active (passive mode - port may still receive data)');
  }
...
const defaultScaleConfig = {
  ...
  parity: 'none',
  stopBits: 2,
  timeoutMs: 2000, // Increased to 2000ms - some scales need more time to respond
  ...
  passiveMode: false, // If true, read data continuously without sending poll commands (for scales that auto-send)
  pollCommand: 'P\r'  // Command to send for polling (can be changed per scale model)
};
...
function loadScaleConfig() {
  try {
    if (fs.existsSync(SCALE_CONFIG_FILE)) {
      ...
      console.log('📋 Scale config loaded:', {
        port: mergedConfig.port,
        passiveMode: mergedConfig.passiveMode || false,
        timeoutMs: mergedConfig.timeoutMs || 2000,
        pollCommand: mergedConfig.pollCommand || 'P\\r'
      });
      return mergedConfig;
    }
  } catch (error) {
    logToFile(`⚠️  Failed to load scale config: ${error.message}, using defaults`);
    console.warn('⚠️  Failed to load scale config, using defaults:', error.message);
  }
...
app.post('/api/scale/config', async (req, res) => {
  try {
    ...
    scaleConfig = { 
      ...
      timeoutMs: cfg.timeoutMs !== undefined ? cfg.timeoutMs : (scaleConfig.timeoutMs || 2000),
      passiveMode: cfg.passiveMode !== undefined ? cfg.passiveMode : (scaleConfig.passiveMode || false),
      pollCommand: cfg.pollCommand || scaleConfig.pollCommand || 'P\r'
    };
...
app.get('/api/scale/read', async (req, res) => {
  ...
  // OPTIMIZATION: If continuous reading is active, return cached reading instead of reading port separately
  // This prevents timeout issues when port is already being read by continuous reading
  if (continuousReadingActive && lastScaleReading) {
    return res.json({ 
      success: true, 
      timestamp: new Date().toISOString(),
      ...lastScaleReading,
      source: 'continuous_reading_cache'
    });
  }
  
  // CRITICAL: Check if port is in bad state and needs recovery
  if (activePort && !activePort.isOpen) {
    console.warn('⚠️  Active port is closed, resetting reference for recovery');
    activePort = null;
  }
  ...
  try {
    ...
    if (port && port.path === normalizedPort && port.isOpen) {
      ...
      // CRITICAL: Only send poll command if NOT in passive mode
      if (!scaleConfig.passiveMode) {
        try {
          if (port && port.isOpen) {
            const pollCmd = scaleConfig.pollCommand || 'P\r';
            port.write(pollCmd);
            if (DEBUG_SCALE) console.log(`📤 Sent poll command: ${JSON.stringify(pollCmd)}`);
          }
        } catch (writeErr) {
          if (DEBUG_SCALE) console.debug('Poll write error (ignored):', writeErr.message);
        }
      } else {
        if (DEBUG_SCALE) console.log('📡 Passive mode: Reusing port, waiting for automatic data');
      }
    } else {
      ...
      if (activePort && activePort.isOpen && activePort.path !== normalizedPort) {
        ...
        // Port was closed, reset reference and log for debugging
        console.warn('⚠️  Existing port was closed unexpectedly, will open new one', {
          port: scaleConfig.port,
          possibleCause: 'Port may have been closed by error or another process',
          passiveMode: scaleConfig.passiveMode || false
        });
        activePort = null;
        port = null;
      ...
      // CRITICAL: Also check if port exists but path doesn't match (shouldn't happen but safety check)
      if (activePort && activePort.path !== normalizedPort) {
        console.warn('⚠️  Port path mismatch, resetting', {
          expected: normalizedPort,
          actual: activePort.path
        });
        try {
    ...
    const cleanup = () => {
      try {
        ...
        // CRITICAL: Keep port open for persistent connection
        // In passive mode, we MUST keep port open and listeners active to receive automatic data
        // Only remove listeners if continuous reading is not active AND not in passive mode
        if (port && !continuousReadingActive && !scaleConfig.passiveMode) {
          // Active mode: Remove listeners after each request (will be re-added on next request)
          port.removeAllListeners('data');
          port.removeAllListeners('error');
          if (DEBUG_SCALE) console.log('🧹 Cleanup: removed event listeners (port kept open for reuse)');
        } else if (port && (continuousReadingActive || scaleConfig.passiveMode)) {
          // Passive mode or continuous reading: Keep listeners active to receive data
          if (DEBUG_SCALE) console.log('🧹 Cleanup: keeping listeners active (passive mode or continuous reading)');
          // DO NOT close port or remove listeners - we need them to receive automatic data
        }
        // IMPORTANT: Never close port here - it should stay open for reuse
      } catch (err) {
    ...
    if (shouldReusePort) {
      ...
      // CRITICAL: Check passive mode setting
      const isPassiveMode = scaleConfig.passiveMode === true;
      console.log(`📡 Port reuse: passiveMode=${isPassiveMode}, port.isOpen=${port.isOpen}`);
      
      // ALTERNATIVE: If passive mode is enabled, don't send poll command - just wait for data
      // Some scales send data automatically without needing poll commands
      if (!isPassiveMode) {
        // Active mode: Send poll command immediately if port is already open
        try {
          if (port && port.isOpen) {
            const pollCmd = scaleConfig.pollCommand || 'P\r';
            port.write(pollCmd);
            console.log(`📤 Active mode: Sent poll command: ${JSON.stringify(pollCmd)}`);
          }
        } catch (writeErr) {
          console.warn('⚠️ Poll write error:', writeErr.message);
        }
      } else {
        console.log('📡 Passive mode: Port reused, waiting for scale to send data automatically (no poll command)');
      }
    }
    ...
    const errorHandler = (err) => {
      ...
      // Auto-recovery: Reset port reference if error occurs
      if (activePort === port) {
        console.warn('⚠️  Resetting activePort reference due to error');
        activePort = null;
      }
      
      if (err.message && err.message.includes('cannot open')) {
        finalize(false, { 
          error: `Port ${scaleConfig.port} is busy or not available. Close other applications using this port.`,
          diagnostic: {
            port: scaleConfig.port,
            suggestion: 'Check if another application is using this COM port, or try restarting the server'
          }
        });
      } else {
        finalize(false, { 
          error: err.message || 'Port error',
          diagnostic: {
            port: scaleConfig.port,
            errorMessage: err.message,
            suggestion: 'Check scale connection and port settings'
          }
        });
      }
    };
    ...
    if (shouldReusePort) {
      ...
      port.open((err) => {
        ...
        const isPassiveMode = scaleConfig.passiveMode === true;
        console.log(`🔌 Opening port: passiveMode=${isPassiveMode}, port=${scaleConfig.port}`);
        ...
        if (err) {
          ...
          finalize(false, { 
            ...
            diagnostic: {
              ...
              errorMessage: err.message,
              suggestions: [
                'Check if port exists in Device Manager',
                'Close other applications using this port',
                'Try unplugging and replugging the scale USB cable',
                'Restart the server if problem persists'
              ]
            }
          });
          return;
        ...
        // ALTERNATIVE MODES:
        // 1. Passive mode: Don't send command, wait for scale to auto-send data
        // 2. Active mode: Send poll command to request data
        if (!scaleConfig.passiveMode) {
          // Active mode: Send poll command
          try {
            if (port && port.isOpen) {
              const pollCmd = scaleConfig.pollCommand || 'P\r';
              port.write(pollCmd);
          ...
          // Passive mode: Just wait for data, don't send commands
          if (DEBUG_SCALE) console.log('📡 Passive mode: Waiting for scale to send data automatically');
        }
      });
    }
    ...
    timeoutId = setTimeout(() => {
      if (!resolved) {
        // Enhanced diagnostic logging for timeout
        const portStatus = port ? {
          isOpen: port.isOpen,
          path: port.path,
          baudRate: port.settings?.baudRate || scaleConfig.baudRate
        } : 'port is null';
        
        console.warn(`⏱️  Timeout after ${scaleConfig.timeoutMs}ms - no data received from scale`, {
          port: scaleConfig.port,
          portStatus: portStatus,
          continuousReadingActive: continuousReadingActive,
          possibleCauses: [
            'Scale may not be powered on',
            'Scale may not be responding to poll command (P\\r)',
            'COM port may be busy or disconnected',
            'Baud rate or serial settings may be incorrect',
            'Scale cable may be loose (check physical connection)'
          ]
        });
        
        finalize(false, { 
          error: `Timeout: No data received from scale after ${scaleConfig.timeoutMs}ms`,
          diagnostic: {
            port: scaleConfig.port,
            portOpen: port?.isOpen || false,
            suggestions: [
              'Check if scale is powered on',
              'Verify COM port connection',
              'Try reconnecting the scale cable',
              'Check Settings > Scale Configuration'
            ]
          }
        });
      }
...
app.post('/api/weighing/save-progress', async (req, res) => {
  ...
  try {
    ...
    const { moNumber, formulationId, ingredients, progress, operatorName } = req.body || {};
...
app.get('/api/history/:mo', async (req, res) => {
  try {
    ...
    const ingredients = allIngredientsResult.rows.map(row => ({
      ...
      sessions: sessionsByIngredient.get(row.ingredient_id) || [] // Empty array if no sessions
    }));
