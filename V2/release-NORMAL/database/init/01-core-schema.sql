...
CREATE TABLE IF NOT EXISTS master_user (
    ...
    role VARCHAR(20) NOT NULL CHECK (role IN ('admin', 'supervisor', 'operator', 'qc')),
    status VARCHAR(20) NOT NULL DEFAULT 'active' CHECK (status IN ('active', 'inactive')),
...
CREATE TABLE IF NOT EXISTS work_orders (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'in_progress', 'completed', 'cancelled', 'reject')),
    created_by UUID NULL REFERENCES master_user(id),
    started_at TIMESTAMP WITH TIME ZONE,
    completed_at TIMESTAMP WITH TIME ZONE,
    qc_reactivate_note TEXT,
    qc_reactivated_by UUID REFERENCES master_user(id),
    qc_reactivated_at TIMESTAMP WITH TIME ZONE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
