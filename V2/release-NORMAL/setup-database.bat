...
echo [1/11] Checking PostgreSQL connection...
...
echo.
echo [2/11] Creating database FLB_MOWS...
...
echo.
echo [3/11] Importing database schema...
...
echo.
echo [4/11] Importing core schema...
set PGPASSWORD=%PGPASSWORD%
...
echo.
echo [5/11] Importing weighing tables...
set PGPASSWORD=%PGPASSWORD%
...
echo.
echo [6/11] Running migration: Add reject status...
set PGPASSWORD=%PGPASSWORD%
...
if exist "%MIGRATION_FILE%" (
    ...
    if !errorlevel! equ 0 (
        ...
        echo ⚠️  Warning: Migration reject status may have failed (non-critical)
    )
) else (
    ...
    if !MIGRATION_SUCCESS! equ 0 (
        echo ⚠️  Migration file not found, but migration may have already been applied
    )
)

echo.
echo [7/11] Running migration: Add opened_at field...
set PGPASSWORD=%PGPASSWORD%
...
if exist "%MIGRATION_FILE%" (
    ...
    if !errorlevel! equ 0 (
        ...
        echo ⚠️  Warning: Migration opened_at may have failed (non-critical)
    )
) else (
    REM Try alternative paths
...
echo.
echo [8/11] Running migration: Add print history table...
set PGPASSWORD=%PGPASSWORD%
...
if exist "%MIGRATION_FILE%" (
    ...
    if !errorlevel! equ 0 (
        ...
        echo ⚠️  Warning: Migration print history may have failed (non-critical)
    )
) else (
    REM Try alternative paths
...
echo.
echo [9/11] Running migration: Add QC role and reactivate note...
set PGPASSWORD=%PGPASSWORD%
...
if exist "%MIGRATION_FILE%" (
    ...
    if !errorlevel! equ 0 (
        ...
        echo ⚠️  Warning: Migration QC role may have failed (non-critical)
    )
) else (
    REM Try alternative paths
...
echo.
echo [10/11] Running migration: Add sequence order...
set PGPASSWORD=%PGPASSWORD%
...
if exist "%MIGRATION_FILE%" (
    ...
    if !errorlevel! equ 0 (
        ...
        echo ⚠️  Warning: Migration sequence order may have failed (non-critical)
    )
) else (
    REM Try alternative paths
...
echo.
echo [11/11] Verifying database setup...
set PGPASSWORD=%PGPASSWORD%
